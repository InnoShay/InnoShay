...
def generate_activity_graph(user: str, out: Path):
    ...
    svg = [
        ...
        f'<text x="22" y="32" font-family="sans-serif" font-size="16" font-weight="bold" fill="{THEME["title"]}">Contribution Activity</text>',
        f'<text x="{width-22}" y="32" font-family="sans-serif" font-size="12" text-anchor="end" fill="{THEME["muted"]}">{sum(days)} contributions in the last year</text>',
        f'<line x1="22" y1="44" x2="{width-22}" y2="44" stroke="{THEME["border"]}"/>',
        
        # The area fill
        f'<path d="{area_d}" fill="url(#grad)" />',
        
        # The line stroke
        f'<path d="{path_d}" fill="none" stroke="{THEME["accent"]}" stroke-width="2" />',
    ]
